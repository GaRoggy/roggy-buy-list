...
function renderTodos(){
  ...
  $("todoList").innerHTML=rows.length?rows.map(x=>{
    ...
    return '<article class="todo-card '+(x.status==="done"?"todo-done":"")+'" data-todo-id="'+esc(x.id)+'"><button class="todo-check" data-todo-toggle="'+esc(x.id)+'" aria-label="'+(x.status==="done"?"Mark open":"Mark complete")+'">'+(x.status==="done"?"✓":"")+'</button><div class="todo-body"><div class="todo-title-row"><h3>'+esc(x.title)+'</h3><span class="todo-priority '+esc(x.priority)+'">'+esc(x.priority)+'</span></div><div class="todo-priority-control"><button type="button" class="todo-priority-btn" data-todo-priority="'+esc(x.id)+'" data-dir="down" aria-label="Lower priority">−</button><span class="priority-control-label">Priority</span><button type="button" class="todo-priority-btn" data-todo-priority="'+esc(x.id)+'" data-dir="up" aria-label="Raise priority">+</button></div><div class="todo-meta">'+(x.category?'<span>'+esc(x.category)+'</span>':"")+todoDueLabel(x)+email+'</div>'+notes+'</div><button class="todo-delete" data-todo-delete="'+esc(x.id)+'" aria-label="Delete task">×</button></article>';
