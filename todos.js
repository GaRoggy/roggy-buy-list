...
function renderTodos(){
  ...
  $("todoList").innerHTML=rows.length?rows.map(x=>{
    ...
    return '<article class="todo-card '+(x.status==="done"?"todo-done":"")+'" data-todo-id="'+esc(x.id)+'"><button class="todo-check" data-todo-toggle="'+esc(x.id)+'" aria-label="'+(x.status==="done"?"Mark open":"Mark complete")+'">'+(x.status==="done"?"✓":"")+'</button><div class="todo-body"><div class="todo-title-row"><h3>'+esc(x.title)+'</h3><div class="todo-priority-control"><button class="todo-priority-btn" data-todo-priority="'+esc(x.id)+'" data-dir="up">↑</button><span class="todo-priority '+esc(x.priority)+'">'+esc(x.priority)+'</span><button class="todo-priority-btn" data-todo-priority="'+esc(x.id)+'" data-dir="down">↓</button></div></div><div class="todo-meta">'+(x.category?'<span>'+esc(x.category)+'</span>':"")+todoDueLabel(x)+email+'</div>'+notes+'</div><button class="todo-delete" data-todo-delete="'+esc(x.id)+'" aria-label="Delete task">×</button></article>';
