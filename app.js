...
function renderBudgetFlow(){
 ...
 groups.forEach(g=>{const gh=Math.max(4,g.value*scale),gc=gy+gh/2,c=cls(g.name),sy0=srcY,sy1=srcY+gh,my0=gy,my1=gy+gh,bulge=Math.min(18,Math.max(4,gh*.12));el("path",{d:`M ${sx+sw} ${sy0} C 270 ${sy0},315 ${my0-bulge},${mx} ${my0} L ${mx} ${my1} C 315 ${my1+bulge},270 ${sy1},${sx+sw} ${sy1} Z`,class:"sk-band "+c});el("rect",{x:mx,y:gy,width:mw,height:gh,rx:5,class:"sk-mid "+c});if(gh>=30){txt(mx+14,gc-3,g.name,"sk-mid-label");txt(mx+14,gc+15,money(g.value)+" · "+(g.value/m.income*100).toFixed(1)+"%","sk-mid-value")}else{txt(mx+mw+8,gc+4,g.name+" "+money(g.value),"sk-tiny-label")}pos.push({...g,y:gy,h:gh,cls:c});gy+=gh+gap;srcY+=gh+gap});
 let iy=top;pos.filter(g=>g.items.length).forEach(g=>{let outY=g.y;g.items.forEach(x=>{const ih=x._value*scale,displayH=Math.max(30,ih),center=iy+displayH/2;el("path",{d:`M ${mx+mw} ${outY} C 655 ${outY},710 ${center-ih/2-6},${rx} ${center-ih/2} L ${rx} ${center+ih/2} C 710 ${center+ih/2+6},655 ${outY+ih},${mx+mw} ${outY+ih} Z`,class:"sk-band "+g.cls});el("rect",{x:rx,y:center-displayH/2,width:rw,height:displayH,rx:6,class:"sk-item"});el("rect",{x:rx,y:center-displayH/2,width:7,height:displayH,rx:3,class:"sk-accent "+g.cls});txt(rx+18,center+4,x.name,"sk-item-label");txt(rx+rw-12,center+4,money(x._value),"sk-item-value");outY+=ih;iy+=displayH+7})});
