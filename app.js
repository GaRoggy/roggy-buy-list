...
function renderBudgetFlow(){
 ...
 const NS="http://www.w3.org/2000/svg",svg=document.createElementNS(NS,"svg"),W=1080,H=660,sx=20,sw=155,mx=455,mw=18,rx=790,rw=265,top=28,bottom=28,groupGap=24,avail=H-top-bottom-groupGap*(groups.length-1),scale=avail/m.income;
 ...
 const el=(tag,attrs,parent=svg)=>{const z=document.createElementNS(NS,tag);Object.entries(attrs||{}).forEach(([k,v])=>z.setAttribute(k,v));parent.appendChild(z);return z},txt=(x,y,v,cls,parent=svg)=>{const z=el("text",{x,y,class:cls},parent);z.textContent=v;return z},cls=n=>"sk-"+n.toLowerCase().replace(/\s+/g,"-");
 const sourceY=top,sourceH=avail+groupGap*(groups.length-1);el("rect",{x:sx,y:sourceY,width:sw,height:sourceH,rx:8,class:"sk-source"});txt(sx+18,sourceY+sourceH/2-18,"Total Income","sk-source-label");txt(sx+18,sourceY+sourceH/2+12,money(m.income),"sk-source-value");txt(sx+18,sourceY+sourceH/2+34,budgetMode==="monthly"?"monthly":"per paycheck","sk-source-sub");
 let gy=top,srcY=sourceY;const pos=[];
 groups.forEach(g=>{const gh=Math.max(3,g.value*scale),gc=gy+gh/2,c=cls(g.name);el("path",{d:`M ${sx+sw} ${srcY} C 285 ${srcY},350 ${gy},${mx} ${gy} L ${mx} ${gy+gh} C 350 ${gy+gh},285 ${srcY+gh},${sx+sw} ${srcY+gh} Z`,class:"sk-band "+c});el("rect",{x:mx,y:gy,width:mw,height:gh,rx:4,class:"sk-mid "+c});txt(mx+mw+12,gc-2,g.name,"sk-mid-label");txt(mx+mw+12,gc+15,money(g.value)+" · "+(g.value/m.income*100).toFixed(1)+"%","sk-mid-value");pos.push({...g,y:gy,h:gh,center:gc,cls:c});gy+=gh+groupGap;srcY+=gh+groupGap});
 pos.filter(g=>g.items.length).forEach(g=>{const itemGap=7,itemMin=32,needed=g.items.length*itemMin+(g.items.length-1)*itemGap,blockH=Math.max(g.h,needed),iy=Math.max(top,g.center-blockH/2);let outY=g.y;g.items.forEach(x=>{const ih=x._value*scale,boxH=Math.max(itemMin,ih),center=iy+boxH/2;el("path",{d:`M ${mx+mw} ${outY} C 610 ${outY},690 ${center-ih/2},${rx} ${center-ih/2} L ${rx} ${center+ih/2} C 690 ${center+ih/2},610 ${outY+ih},${mx+mw} ${outY+ih} Z`,class:"sk-band "+g.cls});el("rect",{x:rx,y:center-boxH/2,width:rw,height:boxH,rx:6,class:"sk-item"});el("rect",{x:rx,y:center-boxH/2,width:7,height:boxH,rx:3,class:"sk-accent "+g.cls});txt(rx+18,center+4,x.name,"sk-item-label");txt(rx+rw-12,center+4,money(x._value),"sk-item-value");outY+=ih;iy+=boxH+itemGap})});
