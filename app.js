...
async function removeItem(x){if(!confirm(`Remove "${x.item}" from your buy list? This clears it from the list.`))return;statusEl.textContent="Removing…";try{await api("update",{row:x.row,item:"",category:"",priority:"",targetPrice:"",status:"",notes:""});items=items.filter(i=>i.row!==x.row);statusEl.textContent="";render()}catch(e){statusEl.textContent=e.message}}
